...
import { User } from "../src/model/models.js";
...
export async function seed() {
  ...
  // create products with associated diets
  for (const product of products) {
    const createdProduct = await Product.create(product);
    for (const { productId, dietId } of dietsOnProduct.filter(
      (p) => p.productId === createdProduct._id.toString()
    )) {
      await createdProduct.diets.push(createdDiets.id(dietId));
    }
